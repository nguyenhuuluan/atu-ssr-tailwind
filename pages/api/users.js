...
import { getAllUsers, findUserByUsername, createUser } from '../../db';
...
handler
  ...
  .post((req, res) => {
    ...
    if (!username || !password || !name) return res.status(400).send('Missing fields');

    // Here you check if the username has already been used
    const usernameExisted = !!findUserByUsername(req, username);
    if (usernameExisted) return res.status(409).send('The username has already been used');
